...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("AP Lottery Ticket", "APLT") {}
...
contract Lottery is Ownable {
    ...
    address public buybackToken = 0x15F96336487906E4972f018dBb0b632a390A4866;
    ...
    uint256 public nextPercentage = 30;
    uint256 public buybackPercentage = 10;
    uint256 public marketingPercentage = 9;
    ...
    event LotteryTokenChanged(IBEP20 _lotteryToken);
    event BuybackTokenChanged(address buybackToken);
    event PrizePercentageChanged(uint256 prizePercentage);
    event NextPercentageChanged(uint256 nextPercentage);
    event BuybackPercentageChanged(uint256 buybackPercentage);
    event MarketingPercentageChanged(uint256 marketingPercentage);

    constructor(
        IBEP20 _lotteryToken,
        LotteryNFT _lottery,
        uint256 _minPrice
    ) 
    public
    Ownable()
    {
        ...
        lotteryToken.approve(address(uniswapV2Router), uint256(-1));
    }
    ...
    function drawing(uint256 _externalRandomNumber) public onlyOwner {
        ...
        if (totalAmount > 0) {
            lotteryToken.transfer( marketingWallet, totalAmount.mul(marketingPercentage).div(100) );
            lotteryToken.transfer( devWallet, totalAmount.div(100) );
            uint256 buybackAmount = totalAmount.mul(buybackPercentage).div(100);

            address[] memory path = new address[](3);
            path[0] = address(lotteryToken);
        ...
        totalAmount = totalAmount.mul(nextPercentage).div(100);
        maxNumber = 0;
    ...
    function buy(uint256 _price) external inDrawingPhase {
        ...
        totalAmount = totalAmount.add(_price);
        lastTimestamp = block.timestamp;
        lotteryToken.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint256 _numbers) external inDrawingPhase {
        ...
        totalAmount = totalAmount.add(_price.mul(_numbers));
        lastTimestamp = block.timestamp;
        totalPrice = totalPrice.add(_price.mul(_numbers));

        lotteryToken.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            lotteryToken.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            lotteryToken.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        ...
        if (_winningNumbers == lotteryNumbers) {
            ...
            reward = poolAmount.mul(prizePercentage).div(100);
        }
        return reward;
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyOwner {
        lotteryToken.safeTransfer(address(msg.sender), _amount);
    ...
    function setlotteryToken(address _lotteryToken) external onlyOwner {
        lotteryToken = IBEP20(_lotteryToken);
        emit LotteryTokenChanged(lotteryToken);
    }
    ...
    function setPrizePercentage(uint256 _prizePercentage) external onlyOwner {
        require(_prizePercentage + buybackPercentage + marketingPercentage + nextPercentage <= 99);
        prizePercentage = _prizePercentage;
        emit PrizePercentageChanged(prizePercentage);
    }

    function setNextPercentage(uint256 _nextPercentage) external onlyOwner {
        require(prizePercentage + buybackPercentage + marketingPercentage + _nextPercentage <= 99);
        nextPercentage = _nextPercentage;
        emit NextPercentageChanged(nextPercentage);
    }

    function setBuybackPercentage(uint256 _buybackPercentage) external onlyOwner {
        require(prizePercentage + _buybackPercentage + marketingPercentage + nextPercentage <= 99);
        buybackPercentage = _buybackPercentage;
        emit BuybackPercentageChanged(buybackPercentage);
    }

    function setMarketingPercentage(uint256 _marketingPercentage) external onlyOwner {
        require(prizePercentage + buybackPercentage + _marketingPercentage + nextPercentage <= 99);
        marketingPercentage = _marketingPercentage;
        emit MarketingPercentageChanged(marketingPercentage);
    }
